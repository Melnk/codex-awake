...
public struct DiagnosticsSnapshot: Equatable, Sendable {
    ...
    public var protectionProfile: ProtectionProfileID = .work
    public var automationRules = ProtectionAutomationRules()
    public var automationDecision = ProtectionAutomationDecision(shouldProtect: false)
    public var isOnExternalPower = false
    public var protectionStatistics = ProtectionStatisticsSnapshot()
    ...
    public var sanitizedText: String {
        ...
        let turns = activity.activeTurnKeys.sorted {
            ...
            .joined(separator: ", ")
        let automationBlockers = automationDecision.blockers.map(\.rawValue).joined(separator: ", ")
        return """
            CodexAwake diagnostics
            App version: \(appVersion)
            macOS: \(macOSVersion)
            Architecture: \(architecture)
            Codex path: \(codexPath.map(SafeDisplay.sanitizedText) ?? "not found")
            ...
            Automation profile: \(protectionProfile.rawValue)
            Automation trigger: \(automationRules.trigger.rawValue)
            Automation decision: \(automationDecision.shouldProtect ? "protect" : "sleep allowed")
            Automation blockers: \(automationBlockers.isEmpty ? "none" : automationBlockers)
            External power required: \(automationRules.requiresExternalPower ? "yes" : "no")
            External power connected: \(isOnExternalPower ? "yes" : "no")
            Schedule enabled: \(automationRules.schedule.isEnabled ? "yes" : "no")
            Selected project filters: \(automationRules.selectedProjectPaths.count)
            Auto-stop after tasks: \(automationRules.automaticallyStopsAfterTasks ? "yes" : "no")
            Protected time: \(Int(protectionStatistics.protectedSeconds)) seconds
            Sleep-prevention sessions: \(protectionStatistics.sleepPreventionSessions)
            ...
            Transport: \(transport)
            Endpoint: \(endpoint.map(SafeDisplay.sanitizedText) ?? "unavailable")
            ...
            Closed-Lid error: \(closedLidProtection.lastError.map(SafeDisplay.sanitizedText) ?? "none")
            ...
            Last safe error: \(lastSafeError.map(SafeDisplay.sanitizedText) ?? "none")
