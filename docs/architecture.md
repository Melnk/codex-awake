...
The 1.9 presentation layer keeps layout policy out of the power and chat state machines. `CockpitView` selects the native resizable split layout above 900 points and a two-section Overview/Chat layout below it. `OnboardingView` owns only the versioned first-run flow and delegates every saved choice to `AppModel`; `PremiumToggleCard` provides one keyboard- and VoiceOver-accessible full-area interaction for all custom switches. System, light, and dark appearance resolve through `InterfaceTheme`, while the compact Menu Bar preference remains in the typed preferences store. Motion-sensitive transitions honor the system Reduce Motion setting.

The 2.0 automation layer is split into typed policy, platform inputs, persistence, and presentation. `ProtectionAutomationEngine` is a side-effect-free value service that evaluates a `ProtectionAutomationContext` against saved rules for Codex presence or active tasks, external power, overnight schedules, and canonical project-folder boundaries. `SystemPowerSourceProvider` is the only IOKit power-source adapter. `ProtectionStatisticsStore` is an actor that owns its private `0600` JSON state and never counts application downtime. `SharedStatusStorage` publishes only a minimal App Group snapshot for the WidgetKit extension. Profiles are immutable presets; applying one writes the complete automation and power configuration before reconciliation.

`AutomationCommandCenter` is the main-actor bridge used by App Intents and URL/AppleScript commands. It cannot access chat text or files. `Package.swift` asks the Swift compiler to extract constant values for App Intents during release builds; `scripts/build_app.sh` converts those values into `Metadata.appintents`, embeds the separately signed WidgetKit extension, and then signs the complete app. Hotkeys call the same `AppModel` methods, so UI, Shortcuts, AppleScript, and keyboard actions share one policy path.

`CodexDesktopRolloutScanner` enumerates local JSONL rollouts, decodes only the first `session_meta` record, accepts only root records with `source = vscode` and `originator = Codex Desktop`, and searches backwards in bounded chunks for the latest exact `task_started` or `task_complete` marker. It does not parse message contents. File size, modification date, metadata, and the latest marker are cached; subsequent polls inspect only appended bytes, keeping the two-second monitor off the UI thread. Markers older than the current Desktop app launch are ignored so a crash cannot create permanent false activity.

`AwakeCoordinator` combines exact managed-task activity, detected active Codex Desktop sessions, optional full-time Codex Desktop presence, and the final evaluated automation demand. When automation demand is present it is authoritative; the master switch still releases everything immediately. `PowerProtectionManager` routes that single aggregate decision to the unprivileged `PowerAssertionManager` and, when explicitly enabled, `ClosedLidLeaseManager`. The former independently owns zero or one `PreventUserIdleSystemSleep` assertion and zero or one `PreventUserIdleDisplaySleep` assertion according to saved user policy; the latter owns zero or one renewable helper lease. Configuration changes reconcile live assertions transactionally. A one-second idle debounce avoids churn. Unknown managed connection state uses a 30-second grace limit. Confirmed server termination clears the managed input but retains protection when another allowed source still requires it; application termination releases every assertion and lease.

## Privileged Closed-Lid boundary

The main app never executes `pmset` and never runs as root. `CodexAwakeClosedLidHelper` is a small launch daemon embedded under `Contents/Library/PrivilegedHelperTools`. An explicit script opened in Terminal uses `sudo` once to copy that fixed binary and its fixed manifest to root-owned system locations; the password is handled only by `sudo` and is never visible to the app. The XPC adapter, lease store, validator, and fixed `pmset` adapter are separate components. The Mach service uses a four-method interface: status, acquire, renew, and release. The installed manifest pins the client to the Apple Team ID or the exact CDHash of the approved local build; there is no arbitrary command, file, shell, or path parameter.

The client lease lasts at most 120 seconds and renews every 30 seconds only while the aggregate power protection is held. Failed XPC operations are retried with capped exponential backoff. On the first lease, the daemon snapshots the prior `disablesleep` setting, persists the restoration data plus expiry timestamps in a root-only file, and invokes the fixed `/usr/bin/pmset -a disablesleep 1` command. Final release, graceful daemon termination, uninstall, or expiry restores the captured setting. Persisted expiries allow a relaunched daemon to recover after a crash without creating an indefinite system policy change. Developer ID builds authorize a stable Apple Team ID; local development falls back to an exact CDHash requirement and needs one repair after an app update.
...
SwiftPM builds the main executable, helper executable, WidgetKit executable, and tests. `scripts/build_app.sh` assembles a standard foreground app bundle with `Info.plist`, `LSUIElement=false`, semantic/build versions, the `com.melnikoleg.CodexAwake` identifier, App Intents metadata, AppleScript and installer resources, a separately signed helper and widget extension, and final ad-hoc app signing. `Package.swift` opens directly in Xcode; no project generator is required.
...
- Missing charger, active task, schedule window, or selected project: automation returns a typed blocker, acquires no assertion/lease, and the Closed-Lid card explains the exact recovery action.
