...
The main app never executes `pmset` and never runs as root. `CodexAwakeClosedLidHelper` is a small launch daemon embedded under `Contents/Library/PrivilegedHelperTools` and registered by macOS through `SMAppService` using the manifest under `Contents/Library/LaunchDaemons`. Touch ID confirms the user's setup request when available; macOS owns the separate privileged Background Items approval, so the app never sees an administrator password. The XPC adapter, lease store, validator, and fixed `pmset` adapter are separate components. The Mach service uses a four-method interface: status, acquire, renew, and release. At launch the daemon verifies its containing app bundle and pins its listener to the signed app identifier plus Apple Team ID, or to the exact bundle CDHash for a local ad-hoc build; there is no arbitrary command, file, shell, or path parameter.
